...
void TransOne(char *StringLine)	//将二维数组input变为一维数组储存到StringLine 
{
	int n=0;
	int i=0;
...
{	
	...
	MovePen(winwidth-setwhite,set_y); //'winwidth-setwhite"等于最右侧的x值 
	DrawLine(0,-winwidth);
...
int Search(char *Text,int *Pxy)   //查找指定内容，数组奇数项为x坐标，即列数，偶数项0,2,4为y坐标，即行数 
{
	int i=0,j=0;
	int m;
	int c=0;
...
void Replace(char *replacement) 	//替换指定内容，开始替换位置（xy同上），被替换的内容，替换进的内容；若位置处被替换内容不符合则不会替换 
{
	int i=0,j=0;
	char StringLine[800]={};
	char StringTmp[400]={}; 
...
void Main()
{
	...
	h = fH*2;									// 单行文本高度 
	set_y=winheight-fH*3;						//set_y是第一行的高度 
